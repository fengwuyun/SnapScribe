...
export function ProjectDetailPage({ projectId, onBack, onConfigureAI }: { projectId: string; onBack: () => void; onConfigureAI: () => void }) {
  ...
  async function saveActionItems(items: NonNullable<ProjectDetail["summary"]>["actionItems"]) {
    const summary = await ipc.projectActionItemsSave(projectId, items);
