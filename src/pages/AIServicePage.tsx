...
import { promoteModel } from "@/lib/aiModelWorkflow";
...
export function AIServicePage({ focusModels = false, returnToProjectId, onReturn }: { focusModels?: boolean; returnToProjectId?: string; onReturn: (projectId: string) => void }) {
  ...
  async function setDefault(model: AIModelConfig) {
    if (!config) return;
    setMessage("");
    try {
      if (!model.enabled) await ipc.aiModelSetEnabled(model.id, true);
      setConfig(await ipc.aiModelReorder(promoteModel(config.models.map((item) => item.id), model.id)));
      setMessage("默认模型已保存");
    } catch (error) { setMessage(String(error)); await load(); }
  }
    ...
    <section ref={modelsRef} className="mt-6 scroll-mt-8 rounded-lg border border-line bg-surface p-6 shadow-sm">
      ...
      {config.models.length === 0 ? <div className="mt-6 flex flex-col items-center rounded-lg border border-dashed border-line py-12 text-center"><span className="flex size-12 items-center justify-center rounded-round bg-primary-soft text-primary"><Sparkles className="size-6" /></span><p className="mt-4 font-semibold">还没有可用模型</p><p className="mt-1 text-sm text-text-secondary">支持官方、第三方代理及 Ollama 等 OpenAI 兼容服务。</p><Button className="mt-5" onClick={() => setDialogOpen(true)}>添加第一个模型</Button></div> : <div className="mt-5 overflow-hidden rounded-lg border border-line">
        {config.models.map((model) => {
          ...
          return <div key={model.id} draggable onDragStart={() => setDraggingId(model.id)} onDragEnd={() => setDraggingId(null)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { const bounds = event.currentTarget.getBoundingClientRect(); void moveTo(model.id, event.clientY > bounds.top + bounds.height / 2); }} className={`grid min-h-16 grid-cols-[36px_44px_minmax(140px,1.4fr)_minmax(120px,1fr)_minmax(110px,1fr)_88px_168px] items-center gap-2 border-b border-divider px-3 transition-colors last:border-b-0 ${draggingId === model.id ? "bg-primary-soft opacity-60" : "hover:bg-bg"}`}>
            <GripVertical className="size-4 cursor-grab text-text-tertiary active:cursor-grabbing" aria-label="拖动调整模型顺序" /><span className="text-xs font-mono text-text-tertiary">{model.order + 1}</span>
